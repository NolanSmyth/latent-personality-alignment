...
#SBATCH --job-name=lpa-toward-only-ipip14
#SBATCH --output=logs/slurm/toward_only_%j.out
#SBATCH --error=logs/slurm/toward_only_%j.err

# =============================================================================
# Toward-only loss ablation on IPIP-14 mixed data
#
# Hypothesis: setting away=0 removes the gradient cancellation between
# positive ("I agree") and negative ("I disagree") items, so the defense
# can finally overfit the mixed-valence IPIP-14 set.
#
# Loss config:  adv_toward=1.0, adv_away=0.0
#               def_toward=1.0, def_away=0.0
#
...
PROJECT_NAME="lpa-toward-only-ipip14"
BATCH_SIZE=4
LAT_CONFIG="latent_at/lat_config_fewer_steps.json"

echo "============================================================"
echo "Toward-only IPIP-14 mixed training"
echo "  adv: toward=1.0, away=0.0"
echo "  def: toward=1.0, away=0.0"
...
echo ""
echo "============================================================"
echo "Probing checkpoint 50 — does toward-only training overfit?"
echo "============================================================"
